...
EOT;
  	...
  	$Query = <<<EOT
SELECT ROUND(MAGGIACL.PROGQTACAR-MAGGIACL.PROGQTASCA+MAGGIACL.PROGQTARET, 2) AS GIACENZA, MAGGIACL.LOTTO, LOTTI.U_NOCE 
FROM MAGGIACL LEFT OUTER JOIN LOTTI ON LOTTI.CODICE = MAGGIACL.LOTTO AND LOTTI.CODICEARTI = MAGGIACL.ARTICOLO
WHERE MAGGIACL.ARTICOLO = "$codice" AND MAGGIACL.MAGAZZINO = "$maga" 
ORDER BY LOTTO DESC
EOT;
  	$rs = db_query($conn, $Query) or die(mysql_error());
  	...
  	while ($row = mysql_fetch_object($rs))	{
  		if($row->GIACENZA > 0) {
  			...
  			print(">" . $row->LOTTO . _(" - Giac.:") . $row->GIACENZA ." ".$umP. ($row->U_NOCE == 1 ? "Orig. NON CEE" : "")  ."</option>\n");
  		}
  	}
  	if($copy > 0 and !$found and $rw->LOTTO == "0") {
  		//print("<option value=\"" . $rw->LOTTO . "\" selected=\"selected\">" . $rw->LOTTO . "</option>\n");
  		$Query =  <<<EOT
SELECT ROUND(GIACINI+PROGQTACAR-PROGQTASCA+PROGQTARET, 2) AS GIACENZA FROM MAGGIAC 
WHERE ARTICOLO = "$codice" AND MAGAZZINO = "$maga" AND ESERCIZIO = "$anno"
EOT;
