...
while($row = db_fetch_row($queryexe)) {
    ...
    $qx1 = db_query($connectionstring, $Query) or die(mysql_error());
	while($rw1 = db_fetch_row($qx1)) {
		$progr += ($rw1[1] > 0 || $rw1[3] > 0 ? $rw1[0] : -$rw1[0]);
		print ("\t\t\t<aw:magmov>\n");
		print("\t\t\t\t<aw:codice>$lastart</aw:codice>\n");
		print ("\t\t\t\t<aw:datamov>" . $rw1[4] . "</aw:datamov>\n"); 
		print ("\t\t\t\t<aw:rif>" . $rw1[5] . "</aw:rif>\n"); 
		print ("\t\t\t\t<aw:qtacar>" . ($rw1[1] > 0 || $rw1[3] > 0 ? xRound($rw1[0]) : "0") . "</aw:qtacar>\n"); 
		print ("\t\t\t\t<aw:qtasca>" . ($rw1[2] > 0 || $rw1[3] < 0 ? xRound($rw1[0]) : "0") . "</aw:qtasca>\n"); 
		print ("\t\t\t\t<aw:lotto>" . $rw1[6] . "</aw:lotto>\n"); 
		print ("\t\t\t</aw:magmov>\n");
	}
} 
