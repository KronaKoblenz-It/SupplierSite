...
for($n = 1; $n <= $count; $n++) {
	...
	if($row->QUANTITA != $_POST["qta$n"]) {
		...
		//CONTROLLO CHE LA RNC SIA STATA CREATA
		if(abs($qta)/$row->QUANTITA > 0.03){
			print("Controllo che sia stata generata una RNC...<br />");
			$Query =  "SELECT U_IDRESO FROM W_ISORNC WHERE U_IDRESO =" . $_POST["r$n"];
			$rsl = db_query($conn, $Query) or die(mysql_error());
			if(mysql_num_rows($rsl)){
				$goOn = true;
			} else {
				print("RNC non trovata...<br />");
				$nonCaricato = true;
			}
		} else {
			$goOn = true;
...
if(0 == ($id_testa % 100)) {
	...
	if ($nonCaricato){
		$esito = _("Uno o più documenti non caricati, RNC richiesta e non trovata!");
	} else {
...
print("<br>\n");
print("<input type='button' value='Chiudi Pagina' onclick='self.close()'>");
...
if(!empty($codicearti)){
  ...
  webMovs::insWebMov($id_testa, $id, $id, $tipodoc, date('Y-m-d'), $codicearti, $lotto, $qta, $magP, $magA, $tipodoc);
}
