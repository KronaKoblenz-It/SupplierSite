...
$objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('G2', 'Lotto Obbligatorio');
...
while($rw = mysql_fetch_object($queryexe)) {
	...
	if($rw->LOTTI > 0) {
		...
		$isLotti = false;
		writeRow($row, $maga, $art, $desc, $um, "", $isLotti);
		$row++;
	}
}
...
$objPHPExcel->getActiveSheet()->getStyle("E2:F$row")
...
function writeRow($row, $maga, $art, $desc, $um, $lotto, $isLotti) {
global $objPHPExcel;
	$objPHPExcel->setActiveSheetIndex(0)
