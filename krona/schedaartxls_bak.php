...
while($rw = mysql_fetch_object($queryexe)) {
	...
	if($rw->LOTTI > 0) {
		...
		while($rs = db_fetch_row($qe)) {
			writeRow($row, $maga, $art, $desc, $um, xRound($rs[1]), $rs[0], $isLotti, $peso);
			$row++;
		}
	} else {
		$isLotti = false;
		...
		$qe = db_query($connectionstring, $Query) or die(mysql_error() );
		$rs = db_fetch_row($qe);
		writeRow($row, $maga, $art, $desc, $um, xRound($rs[0]), "", $isLotti, $peso);
		$row++;
	}
}
...
function writeRow($row, $maga, $art, $desc, $um, $giac, $lotto, $isLotti, $peso) {
global $objPHPExcel;
	$objPHPExcel->setActiveSheetIndex(0)
