...
$Query .= " CODALT.CODARTFOR, CODALT.U_BARCODE, DOCRIG.U_CLIVEN, ANAGRAFE.DESCRIZION AS DESCLIVEN ";
$Query .= " FROM DOCRIG LEFT OUTER JOIN CODALT ON CODALT.CODICEARTI = DOCRIG.CODICEARTI AND CODALT.CODCLIFOR = DOCRIG.U_CLIVEN ";
$Query .= " LEFT JOIN ANAGRAFE ON DOCRIG.U_CLIVEN = ANAGRAFE.CODICE ";
...
while($row = db_fetch_row($queryexe)) { 
  ...
  print("<cliven>" . (is_null($row[4]) ? "" : $row[4]) . "</cliven>\n");
  print("<descliven>" . (is_null($row[5]) ? "" : substr(str_replace('&','AND',$row[5]),0,20)) . "</descliven>\n"); //substr(str_replace('&','AND',$row[5]),0,10))
}
print("</artinfo>\n");
