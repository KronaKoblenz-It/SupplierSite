...
$cf = $cookie[0];
$art = $_GET["art"];
$codLot = isset($_GET['lotto']) ? $_GET['lotto'] : '' ;
$maga = "F" . substr($cf,2);
...
if(empty($codLot)){
	$Query = "SELECT DOCRIG.ID, DOCRIG.DATADOC, DOCRIG.TIPODOC, DOCRIG.NUMERODOC, DOCRIG.CODICECF,
							DOCRIG.LOTTO, DOCRIG.QUANTITA, ANAGRAFE.DESCRIZION
						FROM DOCRIG INNER JOIN DOCTES ON DOCRIG.ID_TESTA = DOCTES.ID
							INNER JOIN ANAGRAFE ON ANAGRAFE.CODICE = DOCRIG.CODICECF
						WHERE DOCTES.MAGARRIVO = '$maga'
							AND (DOCRIG.TIPODOC='BT' or DOCRIG.TIPODOC='CE' or DOCRIG.TIPODOC='RL' or DOCRIG.TIPODOC='TL')
							AND DOCRIG.CODICEARTI = '$art'
						ORDER BY DATADOC ";
} else {
	$Query = "SELECT DOCRIG.ID, DOCRIG.DATADOC, DOCRIG.TIPODOC, DOCRIG.NUMERODOC, DOCRIG.CODICECF,
							DOCRIG.LOTTO, DOCRIG.QUANTITA, ANAGRAFE.DESCRIZION
						...
						WHERE DOCTES.MAGARRIVO = '$maga'
							AND (DOCRIG.TIPODOC='BT' or DOCRIG.TIPODOC='CE' or DOCRIG.TIPODOC='RL' or DOCRIG.TIPODOC='TL')
							AND DOCRIG.CODICEARTI = '$art' AND DOCRIG.LOTTO = '$codLot'
						ORDER BY DATADOC ";
}
$queryexe = db_query($connectionstring, $Query) or die(mysql_error());
//print($Query);
				...
				<tfoot>\n
					<tr class='list'>\n
						<th class='list' colspan='6'>\n
							<input type='submit' value='" . _("Invia rettifiche") . "'>\n
			");
hiddenField("count",$n);
print("			</th>\n
					</tr>\n
