...
if ($cUMPr.trim() == 'CF') {
    ...
    } elseif ('PZ' == $row->UNMISURA2) {
        $numpack = floor(1 / $row->FATT2);
    } elseif ('PZ' == $row->UNMISURA3) {
    ...
    if ('CF' == $row->UNMISURA1) {
        $numpack = $row->FATT1;
    } elseif ('CF' == $row->UNMISURA2) {
        $numpack = $row->FATT2;
    } elseif ('CF' == $row->UNMISURA3) {
        $numpack = $row->FATT3;
    } else {
        $numpack = $row->QTACONF;
    }
...
if ($numpack == 0) {
    //pu� essere che non ci sia la SC o che sia valorizzata a 0
  $numpack = 1;
}
$lCE = $row->U_CE;
$lPEFC = $row->U_PEFC;
...
$pdf->SetFont('Arial', '', '8');
    $pdf->Text(9, 68, ucfirst(strtolower($cDesc)));
//      $pdf->SetXY(0, 60);
