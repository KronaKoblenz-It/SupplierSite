		...
		for ($row = 2; $row <= $highestRow; ++ $row) {
			...
			if (!$ERR_ART_P && ($art != $artP)){
				...
				if(!$ERR_ART_C){
					...
					if ($isLotto){
						...
						$varGiac=$giac+webMovs::giacWebMov($maga, $art, $lottoC);
						//print("$art -> $varGiac");
						...
						if($rwg = mysql_fetch_object($rg)) {
							...
							$varGiac=$varGiac+webMovs::giacWebMov($maga, $art, '');
							if ($consumoArtC > $varGiac){
			...
			if(!$err){
				if( $ERR_ART_C || $ERR_ART_P || $ERR_LOT_C || $ERR_GIAC || $ERR_QTA_C ){
					print("<br>$artP - $art - $lottoC <br>");
					print("<br>ERR_ART_P = $ERR_ART_P <br> ERR_ART_C = $ERR_ART_C <br> ERR_QTA_C = $ERR_QTA_C <br> ERR_LOT_C = $ERR_LOT_C <br> ERR_GIAC = $ERR_GIAC <br> ERR_RIF_ORD = $ERR_RIF_ORD <br>");
					$err = true;
				}
			}
			...
			if(!$err){
				...
				if($lastbolla != $bollaNum) {
					...
					$Query .= "(ID, DATADOC, ESERCIZIO, CODICECF, TIPODOC, NUMERODOC, NUMERODOCF, MAGPARTENZ, MAGARRIVO, RIF_TIPODOC, RIF_NUMERODOC, RIF_DATADOC, DEL) VALUES ( ";
					$Query .= "$id_testa, ";
					$Query .= "'" . date("Y-m-d") . "', '" . date("Y") . "', ";
					$Query .= "\"$fornitore\", ";
					$Query .= "\"$tipodoc\", \"\", \"$bollaNum\", ";
					$Query .= "\"$maga\", \"00001\", \"$tipoRifOrd\", \"$numRifOrd\", \"$dataRifOrd\", 2 )";

					$rs = db_query($conn, $Query) or die(mysql_error());
					$lastbolla = $bollaNum;
				}
				if($lastbollaRif != ($bollaNum . $artP . $lottoP . $tipoRifOrd . $numRifOrd . $rifRiga . $qtaArtP)) {

					$lastbollaRif = $bollaNum . $artP . $lottoP . $tipoRifOrd . $numRifOrd . $rifRiga . $qtaArtP;
					$id_riga = $id+1;
					//RIGA DI COMMENTO!!!
					...
					$Query .= "\"$tipodoc\", ";
					$Query .= "\"\", ";
					$Query .= "\"Rif. $tipoRifOrd $numRifOrd del " . $dataRifOrd ."\", ";
					$Query .=" 1, ";
					$Query .= "\"\", ";
					$Query .= '"", ';
					$Query .= "\"$maga\", \"00001\", ";
				...
				//PORTO LA QTA ALL'UNITA MISURA PRINCIPALE COMPONENTE
				$qtaArtC = $qtaArtC * $fattArtC;
				...
				if(!empty($art)){
					if ($tipoDistin == "P"){
						webMovs::insWebMov($id_testa, $id, $id_riga, 'CE', date('Y-m-d'), $art, $lottoP, $qtaArtP, $maga, '00001', $bollaNum);
					} else {
						webMovs::insWebMov($id_testa, $id, $id_riga, 'CE', date('Y-m-d'), $art, $lottoC, $qtaArtC, $maga, '00001', $bollaNum);
					}
		    }

			} else {
				print("<br><div style=\"text-align: center;\"><b>Errore nel caricamento DDT ". $bollaNum ." su Articolo Padre: ". $artP . " e Lotto: " . $lottoP ."</b></div>\n");
				/* perch� eseguire questa pericolosa cancellazione?	
