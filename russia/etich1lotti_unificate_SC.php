...
if("SC" == $row->UNMISURA1) {
	$numpack = $row->FATT1;
} else if("SC" == $row->UNMISURA2) {
	$numpack = $row->FATT2;
} else if("SC" == $row->UNMISURA3) {
	$numpack = $row->FATT3;
} else {
	$numpack = 0;
}
...
if($numpack != 0){ //pu� essere che non ci sia la SC o che sia valorizzata a 0
...
if($lPEFC == true) {
	...
	$pdf->Text(20,78,"100% PEFC Certified-ICILA-PEFC COC-");
	$pdf->Image("../img/loghi/LOGO-PEFC.PNG", 142, 18, 34 );
}
