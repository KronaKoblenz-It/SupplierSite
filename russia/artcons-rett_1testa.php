...
for($n = 1; $n <= $count; $n++) {
	...
	if($row->QUANTITA != $_POST["qta$n"]) {
		if(0 == $id_testa) {
			$id_testa = scriviTesta($fornitore, "SK", $maga, $row->CODICEARTI);
			$id = $id_testa *100;
		}
		// riga di commento
		$id = scriviRiga($id, $id_testa, "SK", "", $fornitore, "", 1, "", $maga, "Rif. " . $row->TIPODOC . " " . $row->NUMERODOC . " del " . format_date($row->DATADOC), "C", $row->ID, $row->ID_TESTA);
		// riga articolo
		$id = scriviRiga($id, $id_testa, "SK", "", $fornitore, $row->CODICEARTI,  $row->QUANTITA - $_POST["qta$n"], $row->LOTTO, $maga, "", $row->U_CLIVEN, $row->ID, $row->ID_TESTA);
...
if(0 == $id_testa) {
	...
	mail("spedizioni@koblenz.it;ced@k-group.com", "Generato documento sfridi da $fornitore",  "Il fornitore $fornitore ha creato un documento SK per l'articolo ".$row->CODICEARTI, "From: automatico@k-group.com");
}
print("<br>$esito.\n<br<\n>");
...
function scriviTesta($fornitore, $tipodoc, $maga, $rif) {
global $conn;
