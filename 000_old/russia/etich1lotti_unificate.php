...
if($cCodice != ""){
    ...
    if($row->CODICE == ""){
        ...
        $Query = "SELECT CODICEARTI FROM CODALT WHERE CODARTFOR = \"$cCodice\"";
        $queryexe = db_query($conn, $Query);
        $row = mysql_fetch_object($queryexe);
        $cCodiceAlt = $cCodice;
...
if("" == $code) {
	$Query = "SELECT ALIAS FROM MAGALIAS WHERE CODICEARTI = \"$cCodice\""; 
	$queryexe = db_query($conn, $Query) or die(mysql_error()); 
	$row = mysql_fetch_object($queryexe);
	$code = $row->ALIAS;
}
...
$numpack = 0;
$Query = "SELECT QTACONF FROM MAGART WHERE CODICE = '" . $cCodice . "'";
$queryexe = db_query($conn, $Query) or die(mysql_error());
$row = mysql_fetch_object($queryexe);
$numpack = $row->QTACONF;
...
$pdf->Image("etichetta.jpg", 10, 7, 100 );
...
$pdf->SetFillColor(255,255,255);
$pdf->Rect(15,22,43,8,'F');
$pdf->Rect(62,22,43,8,'F');
$pdf->Text(5, 30, $ean13_1);
$pdf->Text(22, 30, $ean13_2);
$pdf->Text(68, 30, $ean13_3);
...
$pdf->SetFont('Arial','B','6');
$pdf->Text(120, 28, "For.: $codkrona");

// -------------------------------------------------- //
//                      LOGO
// -------------------------------------------------- //
if($pathlogo == "../img/loghi/LOGO-KRONA.jpg"){
    $pdf->Image($pathlogo, 210, 7, 35 );
}
else{
    $pdf->Image($pathlogo, 190, 7 ,55);
}
...
$pdf->SetFont('Arial','B','12');
if($cCodiceAlt == ""){
    $pdf->Text(20, 94, $cCodice);
}
else{
    $pdf->Text(20, 94, $cCodiceAlt);
}
