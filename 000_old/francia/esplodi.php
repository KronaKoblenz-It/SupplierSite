...
$rif_riga = $_GET['rif'];
$Query = "SELECT DOCTES.TIPODOC FROM DOCTES LEFT JOIN DOCRIG ON DOCTES.ID = DOCRIG.ID_TESTA WHERE DOCRIG.ID = " . $rif_riga;
...
if($tipodoc == "OF")
{
    //NON devo esplodere la distinta
    mostraConferma($articolo, $quantita, $row->DESCRIZION, $_GET['lotto']);
}
else
{
...
function mostraConferma($articolo, $quantita, $descrizion, $lottopadre, $idRiga){
    global $anno, $conn, $tipodoc;
    ...
    print("<input type='hidden' name='tipodoc' id='tipodoc' value='$tipodoc'>\n");
    print("<h2>Conferma inserimento documento.</h2>");
