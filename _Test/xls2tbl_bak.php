...
//$string = implode($contents); 
	...
	foreach ($objPHPExcel->getWorksheetIterator() as $worksheet) {
		...
		for ($row = 2; $row <= $highestRow; ++ $row) {
			...
			if(!$err){
				if( $ERR_ART_C || $ERR_ART_P || $ERR_QTA_C || $ERR_LOT_C || $ERR_GIAC || $ERR_RIF_ORD ){
					$err = true;
				}				
			}
...
*/
	...
	print("<div style=\"text-align: center;\"><form action=\"xls2doc.php\" method=\"post\" enctype=\"multipart/form-data\">\n");
